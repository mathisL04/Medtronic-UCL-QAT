...
#
#   nohup bash results/experiments/qat_iteration_2/run_campaign.sh > results/experiments/qat_iteration_2/sweeps/CAMPAIGN.log 2>&1 &
set -u
...
S="$REPO/results/experiments/qat_iteration_2/sweeps"
    ...
    if [ -z "$cur" ] || done_log "$cur"; then          # this GPU is free
      ...
      while [ $qi -lt ${#QUEUE[@]} ]; do
        ...
        env VERSION="$ver" DEVICE="$g" $extra $COMMON \
          nohup bash results/experiments/qat_iteration_2/run_version.sh > "$S/${ver}_run.log" 2>&1 &
        ON[$g]="$ver"
...
KNOB=lr0 VERSIONS=V2_lr0_1e-4,V2_lr0_3e-4,V2_lr0_1e-3,V2_lr0_3e-3,V2_lr0_1e-2 "$PY311" results/experiments/qat_iteration_2/gen_sweep_doc.py || true
KNOB=lrf VERSIONS=V2_lrf_0.001,V2_lrf_0.1 "$PY311" results/experiments/qat_iteration_2/gen_sweep_doc.py || true
KNOB=batch VERSIONS=V2_batch_8,V2_batch_32 "$PY311" results/experiments/qat_iteration_2/gen_sweep_doc.py || true
KNOB=n_calib VERSIONS=V2_ncalib_32,V2_ncalib_512 "$PY311" results/experiments/qat_iteration_2/gen_sweep_doc.py || true
