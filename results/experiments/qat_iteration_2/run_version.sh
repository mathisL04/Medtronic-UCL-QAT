...
#     bash results/experiments/qat_iteration_2/run_version.sh
set -o pipefail
...
OUT="$REPO/results/experiments/qat_iteration_2/sweeps/$VERSION"
mkdir -p "$OUT"
stamp() { date '+%H:%M:%S'; }
...
echo "[$(stamp)] doc"
VERSION="$VERSION" OUT="$OUT" "$PY_TRAIN" results/experiments/qat_iteration_2/gen_version_doc.py
